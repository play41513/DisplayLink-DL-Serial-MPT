...
bool DUT::GetInfo(){
	...
	if(bl){
		//otp
		AnsiString SS = USBHIDForm->DosCommand("dl_mfg_test info otp --dev "+AnsiString(_Dev1));
		SS = SS.SubString(19,SS.Pos("\r")-19).Trim();
		if(SS.Length()>5)
			USBHIDForm->g_OTP1 = SS;
		//
	...
	if(_Kind == 2 && bl){
		...
		if(!bl)
		{
			parameter = _T("mtt --dev ") + _Dev2;
			bl = (Pipe(parameter) == 0L);
		}
		//USBHIDForm->memoMsg->Lines->Add("2222");
		if(bl){
			//otp
			AnsiString SS = USBHIDForm->DosCommand("dl_mfg_test info otp --dev "+AnsiString(_Dev2));
			SS = SS.SubString(19,SS.Pos("\r")-19).Trim();
			if(SS.Length()>5)
				USBHIDForm->g_OTP2 = SS;
			//
...
bool DUT::DockRun(String parameter, bool checkKind){
	...
	if(USBHIDForm->bDebug) USBHIDForm->memoMsg->Lines->Add(cn);
	if(Pipe(cn) != 0L){
		if(!parameter.Pos("video")) return false;
	}
...
bool DUT::ShowVideo1080P(){
	...
	DockRun(_T("video bitmap .\\RGB.bmp -m 1920 1080 60"));
	 // jim 20161102 遇到兩個head的情況
	DockRun(_T("video bitmap .\\RGB.bmp -m 1920 1080 60 --head 1"));
	return true;
}
...
bool DUT::ShowVideo4K(){
	...
	if(DockRun(_T("video bw 1"))){
		Delay(300);
		DockRun(_T("video --detailedmode 4400 3840 176 88 1 2250 2160 8 10 1 300000 --head 0"));
		DockRun(_T("video --detailedmode 4400 3840 176 88 1 2250 2160 8 10 1 300000 --head 1"));// jim 20161102 遇到兩個head的情況
		return true;
	}
	else{
		LOG(LS_WARNING) << "Failed to execute video detailedmode!";
...
bool DUT::ShowVideo4K60(){
	...
	if(DockRun(_T("video bw 1"))){
		Delay(300);
		DockRun(_T("video --detailedmode 4400 3840 296 176 1 2250 2160 72 8 1 594000 --head 0"),true);
		DockRun(_T("video --detailedmode 4400 3840 296 176 1 2250 2160 72 8 1 594000 --head 1"),true);
		return true;
	}else{
		LOG(LS_WARNING) << "Failed to execute video detailedmode!";
		MessageBox(NULL, "Failed to execute video detailedmode!", "DUT::ShowVideo4K 60", MB_ICONWARNING);

...
//	Sleep(1000);
	...
	memoMsg->Lines->Add("\n\n");
	memoMsg->Lines->Add("SN   : "+sn1);
	if(mac1.Length()>=12)
		mac1 = mac1.SubString(1,6)+":"+mac1.SubString(7,6);
	memoMsg->Lines->Add("MAC  : "+mac1);
	if(m_dut._Kind == 2){
		memoMsg->Lines->Add("\n");
		memoMsg->Lines->Add("SN  2: "+sn2);
		if(mac2.Length()>=12)
			mac2 = mac2.SubString(1,6)+":"+mac2.SubString(7,6);
		memoMsg->Lines->Add("MAC 2: "+mac2);
	}
	memoMsg->Lines->Add("\n");
	return(ERRC);
}
...
void __fastcall TUSBHIDForm::Debug1Click(TObject *Sender)
{
	bDebug = true;
}
//---------------------------------------------------------------------------

void __fastcall TUSBHIDForm::CloseDebug1Click(TObject *Sender)
{
	bDebug = false;
}
//---------------------------------------------------------------------------
void TUSBHIDForm::WriteLog(AnsiString MSG)
{
	...
	if(MSG=="SET")
	{
		...
		AnsiString burn = ckbBurn->Checked ? "true":"false";
		AnsiString verify = ckbBurnVerify->Checked ? "true":"false";
		AnsiString Setting = "[Number of Chip] "+cmbNumOfChip->Text
							+"[自動跳號] "+Auto
							+"[燒錄] "+burn+"\r\n"
							+"[SN] "+mskedtBeginSn->Text
							+"[MAC] "+mskedtMacPre->Text+mskedtBeginMac->Text
							+"[驗證]"+verify;
		strMessage = "\r\n==========[設定值]===============\r\n"+Time +"\r\n"+Setting+"\r\n=================================\r\n";
	}
	else
	{
		BurnCount++;
		AnsiString sPASS = MSG=="PASS"?"PASS":"FAIL";
		strMessage.printf("%04d %s %s", BurnCount,sPASS,Time);

		strMessage += " [SN]"+m_dut._If1.SN + " [MAC]" +m_dut._If1.MAC + " [BackupFW]"+m_dut._If1.BackFirmware+ " [MinFW]"+m_dut._If1.MinFwVersion+" [OTP]"+g_OTP1;
		if(m_dut._Kind == 2){
			strMessage += " [SN2]"+m_dut._If2.SN + " [MAC2]" +m_dut._If2.MAC + " [BackupFW2]"+m_dut._If1.BackFirmware+ " [MinFW2]"+m_dut._If2.MinFwVersion+" [OTP2]"+g_OTP2;
		}
		if(MSG!="PASS") strMessage +=" [ERROR]"+MSG;
	}

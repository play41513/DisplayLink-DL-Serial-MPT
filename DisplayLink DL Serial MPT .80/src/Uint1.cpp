...
bool TUSBHIDForm::CheckWin10Driver()
{
	...
	for(int x=0;x<NList->Count;x++)
	{
		...
		if(!Resultdata.Pos("drivers installed successfully")) return false;
	}
	//重新抓裝置
	...
	{
		NList2->Strings[x] = NList2->Strings[x].SubString(1,27);
		if(!NList2->Strings[x].Pos("USBIO"))
			bResult = false;
	}
	delete NList2;

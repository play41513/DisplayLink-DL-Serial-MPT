...
void __fastcall TUSBHIDForm::btnGetInfo4KClick(TObject *Sender)
{
	...
	if(bResult)
	{
		...
		if(!bl)
		{
			...
			AnsiString Resultdata = DosCommand("dl_mfg_test info otp");
			memoMsg->Lines->Add("============");
...
bool TUSBHIDForm::SaveFile(bool bReault,int Time)
{
	SYSTEMTIME myTime;
	GetLocalTime(&myTime);
	//寫檔內容
	AnsiString FileLine;
	AnsiString temp;
	FileLine.printf("%02d:%02d:%02d  ", myTime.wHour, myTime.wMinute,
		myTime.wSecond);
	for(int i = 0;i<=memoMsg->Lines->Count;i++)
	{
		temp = AnsiString(memoMsg->Lines->Strings[i]);
		if(temp.Pos("MAC address"))
		{
			FileLine = FileLine + " [MAC]";
			if(temp.Length()>15)
			{
				temp = temp.SubString(15,temp.Length());
				FileLine = FileLine + temp;
			}
		}
		else if(memoMsg->Lines->Strings[i].Pos("USB Serial No"))
		{
			FileLine = FileLine + " [SN]";
			if(temp.Length()>17)
			{
		...
		else if(memoMsg->Lines->Strings[i].Pos("OTP Serial Number"))
		{
			FileLine = FileLine + "   ||   [OTP SN]";
			if(temp.Length()>20)
			{
				temp = temp.SubString(20,temp.Length());
				FileLine = FileLine + temp;
			}
		}
		else if(memoMsg->Lines->Strings[i].Pos("OTP SKU"))
		{
			FileLine = FileLine + " [OTP SKU]";
			if(temp.Length()>10)
			{
		...
		else if(memoMsg->Lines->Strings[i].Pos("OTP MAC"))
		{
			FileLine = FileLine + " [OTP MAC]";
			if(temp.Length()>10)
			{
				temp = temp.SubString(10,temp.Length());
				FileLine = FileLine + temp;
	...
	temp = AnsiString(Time);
	temp.printf("%03d",Time);
	FileLine = FileLine + "  [TestTime]"+temp;
	AnsiString TestResult = bReault ? "   PASS":"   FAIL";
	if(!bReault) TestResult = TestResult +"    [Error]"+ErrorCode;
	FileLine = FileLine + TestResult+ "\n";
	//=============寫檔
	FileName.printf("C:\\ASMP\\bin\\DisplayLink DL Serial MPT_4k60\\log\\%02d-%02d-%d.LOG", myTime.wMonth, myTime.wDay,
		myTime.wYear);
	//
	FILE * fp;
		...
		while (pch != NULL)
		{
			...
			else if(strstr(AnsiString(pch).c_str(),"No matching devices found"))
			{
				if(bSaveFile)
				{
					bSaveFile = false;
					bStart = false;
					int Time = (int)((GetTickCount()-TestTime)/1000);
					//
					if(frmMsg==NULL) frmMsg = new TfrmMsg(Application);
					bool bl = frmMsg->ShowModal() == mrOk?true:false;
					ErrorCode = frmMsg->Hint;
					delete frmMsg;
					frmMsg=NULL;
					//
					SaveFile(bl,Time);
